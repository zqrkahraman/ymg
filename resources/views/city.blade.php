...
@section('title', $city_meta_title[app()->getLocale()])
@section('description', $city_meta_description[app()->getLocale()])

@section('content')

    <div class="filter filter-all pt-4">
        <div class="container">
            <h3>Things to do in {{ $city_name[app()->getLocale()] }}</h3>
                    ...
                    @foreach ($cities as $city)
                        @foreach ($city as $item)
                            ...
                            <div class="swiper-slide pb-2">
                                <a href="{{ LaravelLocalization::localizeUrl('/audioguides/'.$country_slug[app()->getLocale()].'/'.$menu_city_slug[app()->getLocale()]) }}" @if($city_slug[app()->getLocale()]==$menu_city_slug[app()->getLocale()]) class="active" @endif> 
    ...
    <div class="pt-md-1 pb-md-5 pt-1 pb-4 shop shop-all">
        <div class="container pt-4">
            <div class="row">
                <div class="title mb-3">
                    <div>
                        <a href="{{ LaravelLocalization::localizeUrl('/audioguides') }}">Audio Guides</a> <i class="fa-solid fa-angle-right"></i> 
                        <a href="{{ LaravelLocalization::localizeUrl('/audioguides/'.$country_slug[app()->getLocale()]) }}">{{ $country_name[app()->getLocale()] }}</a> <i class="fa-solid fa-angle-right"></i> 
                        <strong>{{ $city_name[app()->getLocale()] }}</strong> 
                    </div>
                    <div>
                        {{ count($products) }} {{ __('main.search-results') }}
                    </div>
                </div>
                <div class="product product-horizontal">
                    <div class="container">
                        @foreach ($products as $key => $value)
                            <a href="{{ LaravelLocalization::localizeUrl('/audioguides/'.$country_slug[app()->getLocale()].'/'.$city_slug[app()->getLocale()].'/'.$value['sef_link']) }}">
                                <div class="row mb-3" itemtype="https://schema.org/Product" itemscope>
                                    ...
                                    <div class="col-xl-2 col-lg-2 col-md-4 col-12 p-0 pt-2 text-md-end text-start">
                                        <div class="comments" itemprop="aggregateRating" itemtype="https://schema.org/AggregateRating" itemscope>
                                            ...
                                            <label itemprop="reviewCount">(@if (isset($comments_count[$value->urun_id]))@foreach ($comments_count[$value->urun_id] as $item){{ $item }}@endforeach @endif {{ __('main.reviews') }})</label>
                                        </div>
        ...
        <div class="container pt-4">
            <a href="{{ LaravelLocalization::localizeUrl('/audioguides/'.$country_slug[app()->getLocale()]) }}" class="see-all-bg">
                <div class="see-all">
                    <img src="/images/see-all-3.jpg" alt="{{ __('main.see-all') }}">
                    <div>
                        {{ $country_name[app()->getLocale()] }}
                        <strong>{{ __('main.see-all') }} {{ $country_name[app()->getLocale()] }}</strong>
                    </div>
                </div>
            </a>
        </div>
